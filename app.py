...
def exit_plan(df,total_score,best_strategy):
    ...
    tp1=max(price+1.5*risk,resistance if resistance>price else price+1.5*risk)
    tp2=max(price+2.5*risk,tp1+0.8*atr)
...
def full_analysis():
    ...
    try:
        ...
        rows.sort(key=lambda x:x['score'],reverse=True);best=rows[0];tech,tech_reasons=technical_score(df);news=news_for(symbol);market_score=50
        ...
        return jsonify({'symbol':symbol,'quote':quote,'best_strategy':best['strategy'],'strategy_score':best['score'],'technical_score':tech,'technical_reasons':tech_reasons,'news':news,'market_score':market_score,'total_score':total,'verdict':verdict,'strategies':rows,'exit_plan':plan,'method':'70% training / 30% out-of-sample; totaalscore combineert strategie, techniek, nieuws en markt.'})
    except Exception as e:return jsonify(error=str(e)),400
@app.get('/health')
