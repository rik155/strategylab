...
PRESETS = {
    'tech': ['AAPL','MSFT','NVDA','AMD','AVGO','QCOM','INTC','MU','ARM','AMZN','META','GOOGL','TSLA','PLTR','SOUN','CRWD','PANW','SNOW','NET','DDOG'],
    'us_large': ['AAPL','MSFT','NVDA','AMZN','META','GOOGL','GOOG','BRK.B','LLY','AVGO','JPM','V','MA','WMT','XOM','COST','ORCL','NFLX','HD','PG','JNJ','ABBV','BAC','KO','CRM','CVX','MRK','AMD','PEP','TMO','MCD','ACN','CSCO','LIN','WFC','IBM','GE','CAT','DIS','UBER'],
...
def load_config():
    with config_lock:
        ...
        env = [x.strip().upper() for x in os.getenv('WATCHLIST','').split(',') if x.strip()]
        return {'watchlist': env or DEFAULT_WATCHLIST.copy()}
...
def exit_plan(df,total_score,best_strategy):
    x=enrich(df);r=x.iloc[-1];price=float(r.close);atr=float(r.atr14) if pd.notna(r.atr14) and r.atr14>0 else price*0.025;look=x.tail(60);support=float(look.low.quantile(0.15));resistance=float(look.high.quantile(0.90));stop=max(support,price-1.6*atr);stop=min(stop,price*0.985);risk=max(price-stop,price*0.01);tp1=max(price+1.5*risk,resistance if resistance>price else price+1.5*risk);tp2=max(price+2.5*risk,tp1+0.8*atr);latest=signals(df,best_strategy).iloc[-1]
...
def scanner_watchlist_update():
    ...
    if action=='add' and symbol and symbol not in wl:wl.append(symbol)
        ...
        preset=str(d.get('preset',''));wl=list(dict.fromkeys(wl+PRESETS.get(preset,[])))
